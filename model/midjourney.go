...
func UpdateMidjourneyBillingAttribution(userID int, mjID string, chargedQuota int, allocation WalletAllocation, billingSource string, subscriptionID int, role int, isAdmin bool) error {
	return DB.Model(&Midjourney{}).Where("user_id = ? AND mj_id = ?", userID, mjID).Updates(map[string]interface{}{
		"quota":              chargedQuota,
