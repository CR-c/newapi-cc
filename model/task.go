...
type TaskPrivateData struct {
	...
	WalletAllocation        WalletAllocation    `json:"wallet_allocation,omitempty"`
	WalletAllocationVersion int                 `json:"wallet_allocation_version,omitempty"`
	UserRoleSnapshot        int                 `json:"user_role_snapshot,omitempty"`
	IsAdminUsage            bool                `json:"is_admin_usage,omitempty"`
...
func (p TaskPrivateData) isEmpty() bool {
	return p.Key == "" &&
		...
		p.ResultURL == "" &&
		p.BillingSource == "" &&
		p.WalletAllocation == (WalletAllocation{}) &&
		p.WalletAllocationVersion == 0 &&
		p.UserRoleSnapshot == 0 &&
		!p.IsAdminUsage &&
		p.SubscriptionId == 0 &&
		p.TokenId == 0 &&
		p.NodeName == "" &&
