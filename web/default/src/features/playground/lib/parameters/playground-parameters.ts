/*
Copyright (C) 2023-2026 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as
published by the Free Software Foundation, either version 3 of the
License, or (at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact support@quantumnous.com
*/
import type {
  ParameterEnabled,
  PlaygroundConfig,
  ReasoningEffort,
} from '../../types'

type ParameterValue = PlaygroundConfig[keyof PlaygroundConfig]

export type PlaygroundParameterKey = keyof ParameterEnabled

type SliderOrNumberControl = {
  key: Exclude<PlaygroundParameterKey, 'reasoning_effort'>
  labelKey: string
  descriptionKey: string
  valueType: 'slider' | 'number'
  min: number
  max: number
  step: number
}

type SelectControl = {
  key: 'reasoning_effort'
  labelKey: string
  descriptionKey: string
  valueType: 'select'
  options: readonly ReasoningEffort[]
}

export type PlaygroundParameterControl = SliderOrNumberControl | SelectControl

export const REASONING_EFFORT_OPTIONS = [
  'none',
  'minimal',
  'low',
  'medium',
  'high',
  'xhigh',
] as const satisfies readonly ReasoningEffort[]

export const PLAYGROUND_PARAMETER_CONTROLS = [
  {
    key: 'temperature',
    labelKey: 'Temperature',
    descriptionKey: 'Controls randomness and creativity',
    valueType: 'slider',
    min: 0,
    max: 2,
    step: 0.1,
  },
  {
    key: 'top_p',
    labelKey: 'Top P',
    descriptionKey: 'Limits token selection to a probability mass',
    valueType: 'slider',
    min: 0,
    max: 1,
    step: 0.1,
  },
  {
    key: 'frequency_penalty',
    labelKey: 'Frequency Penalty',
    descriptionKey: 'Reduces repeated wording',
    valueType: 'slider',
    min: -2,
    max: 2,
    step: 0.1,
  },
  {
    key: 'presence_penalty',
    labelKey: 'Presence Penalty',
    descriptionKey: 'Encourages new topics',
    valueType: 'slider',
    min: -2,
    max: 2,
    step: 0.1,
  },
  {
    key: 'max_tokens',
    labelKey: 'Max Tokens',
    descriptionKey: 'Caps the response length',
    valueType: 'number',
    min: 1,
    max: 200000,
    step: 1,
  },
  {
    key: 'max_completion_tokens',
    labelKey: 'Max Completion Tokens',
    descriptionKey: 'Maximum tokens including hidden reasoning tokens',
    valueType: 'number',
    min: 1,
    max: 200000,
    step: 1,
  },
  {
    key: 'reasoning_effort',
    labelKey: 'Reasoning Effort',
    descriptionKey: 'Controls how much the model thinks before answering',
    valueType: 'select',
    options: REASONING_EFFORT_OPTIONS,
  },
  {
    key: 'seed',
    labelKey: 'Seed',
    descriptionKey: 'Keeps compatible responses more repeatable',
    valueType: 'number',
    min: 0,
    max: 2147483647,
    step: 1,
  },
] as const satisfies readonly PlaygroundParameterControl[]

export const PLAYGROUND_PARAMETER_PANEL_SCROLL_CLASS =
  'max-h-[min(28rem,calc(100vh-10rem))] overflow-y-auto pr-1'

export function normalizeParameterNumberValue(
  key: Exclude<PlaygroundParameterKey, 'reasoning_effort'>,
  value: string | number
): number | null {
  const control = PLAYGROUND_PARAMETER_CONTROLS.find(
    (item) => item.key === key && item.valueType !== 'select'
  )

  if (value === '') {
    return key === 'seed' ? null : (control?.min ?? 0)
  }

  const parsed = typeof value === 'number' ? value : Number.parseFloat(value)

  if (!control || control.valueType === 'select' || Number.isNaN(parsed)) {
    return key === 'seed' ? null : (control?.min ?? 0)
  }

  const clamped = Math.min(control.max, Math.max(control.min, parsed))

  if (control.step >= 1) {
    return Math.trunc(clamped)
  }

  const precision = Math.max(0, String(control.step).split('.')[1]?.length ?? 0)
  return Number(clamped.toFixed(precision))
}

export function isReasoningEffortValue(
  value: string
): value is ReasoningEffort {
  return (REASONING_EFFORT_OPTIONS as readonly string[]).includes(value)
}

export function getParameterControlValueText(
  key: PlaygroundParameterKey,
  value: ParameterValue
): string {
  if (key === 'seed' && value === null) {
    return 'Not set'
  }

  return String(value)
}
