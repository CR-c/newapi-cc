/*
Copyright (C) 2023-2026 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as
published by the Free Software Foundation, either version 3 of the
License, or (at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact support@quantumnous.com
*/
import { describe, expect, test } from 'bun:test'

import {
  getParameterControlValueText,
  isReasoningEffortValue,
  normalizeParameterNumberValue,
  PLAYGROUND_PARAMETER_CONTROLS,
} from './playground-parameters'

describe('playground parameters', () => {
  test('exposes the controls that are applied to chat requests', () => {
    const keys = PLAYGROUND_PARAMETER_CONTROLS.map((control) => control.key)
    expect(keys).toEqual([
      'temperature',
      'top_p',
      'frequency_penalty',
      'presence_penalty',
      'max_tokens',
      'max_completion_tokens',
      'reasoning_effort',
      'seed',
    ])
  })

  test('clamps and rounds numeric parameter values', () => {
    expect(normalizeParameterNumberValue('temperature', 2.5)).toBe(2)
    expect(normalizeParameterNumberValue('temperature', -1)).toBe(0)
    expect(normalizeParameterNumberValue('temperature', 0.55)).toBe(0.6)
    expect(normalizeParameterNumberValue('max_tokens', 0)).toBe(1)
    expect(normalizeParameterNumberValue('seed', '')).toBeNull()
    expect(normalizeParameterNumberValue('seed', '12.9')).toBe(12)
  })

  test('validates reasoning effort values', () => {
    expect(isReasoningEffortValue('high')).toBe(true)
    expect(isReasoningEffortValue('ultra')).toBe(false)
    expect(getParameterControlValueText('seed', null)).toBe('Not set')
    expect(getParameterControlValueText('temperature', 0.7)).toBe('0.7')
  })
})
